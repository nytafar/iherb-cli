...
pub async fn cmd_search(
    ...
    if let Some(hit) = cache.get_search::<crate::model::SearchResult>(query, sort, category) {
        let mut result = hit.data;
        result.products.truncate(limit);
        print!("{}", crate::output::format_search_results(&result));
        println!(
            "\n- **Data from:** {}",
            crate::output::format_cached_at(hit.cached_at)
        );
        return Ok(());
    }
    ...
    let total_pages = crate::scraper::search::pages_needed(limit);
    ...
    for page_num in 1..=total_pages {
        ...
        let url =
            crate::scraper::search::build_search_url(&base_url, query, sort, category, page_num);
        let html = navigator
            .navigate_with_retry(&page, &url, 2)
            .await
            .context("Failed to navigate to search page")?;

        let page_result = crate::scraper::search::extract_search(
            &page,
            &html,
            query,
            &base_url,
    ...
    let full_result = crate::model::SearchResult {
    ...
    println!(
        "\n- **Data from:** {}",
        crate::output::format_cached_at(SystemTime::now())
    );
    Ok(())
}
...
pub async fn cmd_product(
    ...
    if let Some(hit) = cache.get_product::<crate::model::ProductDetail>(&product_id) {
        print!(
            "{}",
            crate::output::format_product_detail(&hit.data, section)
        );
        println!(
            "\n- **Data from:** {}",
            crate::output::format_cached_at(hit.cached_at)
    ...
    if crate::scraper::helpers::is_not_found_page(&html) {
        anyhow::bail!("Product not found: {}", product_id);
    }

    let product = crate::scraper::product::extract_product(
        &page,
        &html,
        &product_id,
        &base_url,
    ...
    print!(
        "{}",
        crate::output::format_product_detail(&product, section)
    );
    println!(
        "\n- **Data from:** {}",
        crate::output::format_cached_at(SystemTime::now())
